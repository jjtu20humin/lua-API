...
require "interface.html_pattern";
...
function check_param ( cmd, _GET, _POST, _FILE )
	...
	for __k,v in pairs( _pattern.base_param ) do
		...
		if( (_tmp_vlen < _vlen_min) or (_tmp_vlen > _vlen_max) ) then
			return string.format("Param [%s] length=%d, out of range[%s].", k, _tmp_vlen, _tmp_range );
		end;

		if( type(_vpat) == "string" ) then
			local _vmatch = string.match( _value, _vpat );
			if( _vmatch ~= _value ) then
				return string.format("Param [%s] value, has invalid charactors.", k );
			end;
		elseif ( type(_vpat) == "table" ) then
			...
			if( not _matched ) then
				return string.format("Param [%s] value, out of range[%s].", k, table.concat(_vpat, ",") );
			end;
		end;
	end;

	for __k,v in pairs( _pattern.opt_param ) do
		local k = v.name;
...
function if_route( ) 
    ...
    if( _PATH == 'lapi' ) then
        if type(GV_IFS[_COMMAND]) == 'table' then
			...
			local _result, _reqarray = check_param( _COMMAND, _GET, _postarray, _filearray );
			if( _result ) then
				if( _COMMAND == "svr_getuserinfo" ) then
					ngx.log(ngx.NOTICE, "REQGET:".._JSON(_GET) );
					ngx.log(ngx.NOTICE, "REQPOST:".._JSON(_postarray) );
				end;
				--ngx.log(ngx.NOTICE, "REQ:".._JSON(_reqarray) );
				--ngx.log(ngx.NOTICE, "RESULT".._result );
				_errcode = -1001; -- illegal param
				return _ERR( _errcode, _result );
			end;
			local _mongo_dbh, _mongo_conn = mongo_init();
			if( _mongo_dbh == nil ) then
				return _ERR( -101, "MONGO ERROR" );
			end;
			local _ret = GV_IFS[_COMMAND]['callback']( _mongo_dbh, _reqarray, _filearray );
			mongo_cleanup( _mongo_conn );
			if( _COMMAND == "debug_unshare" ) then
				--ngx.log( ngx.NOTICE, _JSON( _ret) );
			end;
			return _ret;
		else
			_errcode = -1000; -- illegal function
			--ngx.log(ngx.NOTICE, "REQ:".._JSON(_reqarray) );
        end;
    end;
...
function get_html_pattern( mod )
	local _html = "";
	for _line in io.lines( config.root_dir.."interface/html/"..mod..".html" ) do
		_html = _html .. _line;
	end;
...
function get_imsi ( devinfo )
	local _devinfo = devinfo;
        if( _devinfo and #_devinfo >0 ) then
            local _devobj = parse_devinfo( _devinfo );

            if type(_devobj)=="table" then
                return _devobj['imsi'];
            end;
        end;
	return nil;
...
function get_version ( devinfo )
	local _devinfo = devinfo;
        if( _devinfo and #_devinfo >0 ) then
            local _devobj = parse_devinfo( _devinfo );
            if type(_devobj)=="table" then
            	local _ret = _devobj['versionCode'] and tonumber(_devobj['versionCode']) or 0;
                return _ret ;
            end;
        end;
	return 0;
...
function get_channel ( devinfo )
        local _devinfo = devinfo;
        if( _devinfo and #_devinfo >0 ) then
            local _devobj = parse_devinfo( _devinfo );
            if type(_devobj)=="table" then
                local _ret = _devobj['channelId'] or "undefined";
                return _ret ;
            end;
        end;
        return 0;
