...
function parse_header( header )
	local _POST;
	if type(header) == 'table' then			--for wangsu use
		local _headval = header["sender"];
		if _headval and _headval == "FDN" then
			ngx.req.read_body();
			local _t = ngx.req.get_body_data();
			if type(_t) == 'string' then
				_POST = cjson_safe.decode(_t);
			else
				return false;
			end
			return 0, _POST;
...
function proc_multipart_form( cmd )
	...
	if( _form == nil ) then
		--------------------------parse header by bellemere----------------------------
		local _HEADER = ngx.req.get_headers();
		_rcode, _POST = parse_header( _HEADER );
		if _rcode and _rcode == 0 then
			return 0, _POST;
		end
		--------------------------parse header by bellemere----------------------------
	    ngx.req.read_body();
